...
def connect_db(host="localhost", user="root", passwd="Sami@4321", port=3306, database='expense'):
    """
    Connect to database
    :param host: host
    :param user: username
    ...
    try:
        ...
        if err.args[0] == 1045:  # Access denied error
            ...
            print("Please make sure you have run these commands in MySQL:")
            print("ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY 'Sami@4321';")
            print("FLUSH PRIVILEGES;")
        elif err.args[0] == 1049:  # Unknown database error
            print("Database does not exist")
            print("Please run setup_mysql.py first")
        else:
            print(f"Error: {err}")
        raise
...
def execute_query(operation=None, query=None):
    ...
    if operation == 'search':
        ...
        # Convert dictionary results to list format for backward compatibility
        if data and isinstance(data[0], dict):
            data = [[row[col] for col in row.keys()] for row in data]
        cursor.close()
        return data
